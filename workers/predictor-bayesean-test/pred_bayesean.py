...
with pm.Model() as model:
  lambda0 = pm.Gamma('lambda0', mu=0.03, sd=0.01, shape=n_intervals) 
  #lambda0 = pm.Deterministic('lambda0', np.ones(n_intervals)*lambda0_single)
...
num_sample = random.randint(0, n_samples-1) #which sample will I pick?
...
def fitAge_greater_equal_sample(survFn, greaterThan):
  return fitAge_greater_equal(survFn, greaterThan)[num_sample]
...
for i in range(0, num_characters):
  ...
  ch["predictedSurvivalAge"] = fitAge_greater_equal_mean(survFn, 0.5).astype(float).tolist()
  confidence = 0.8
  ch["confIntervalLower"] = fitAge_greater_equal_mean(survFn, confidence).astype(float).tolist()
  ch["confIntervalHigher"] = fitAge_greater_equal_mean(survFn, 1-confidence).astype(float).tolist()
...
for i in range(0, num_characters):
  ...
  if ch['alive'] == False:
    numDead += 1
    error = ch['age']-ch['predictedSurvivalAge']
    meanQuadError += error*error
    if error > 0:
      numUnder += 1
      avgUnder += error
      if error < 10:
...
print(numOver, " characters were overestimated by an average of ", avgOver)
print(numUnder, " characters were underestimated by an average of ", avgUnder)
print(totalClose, " characters were correctly guessed within 10 years")
print(numOutliers, " outliers with more than 20 years of error")
